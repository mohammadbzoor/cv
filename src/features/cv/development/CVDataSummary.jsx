...
import {
  ...
  selectCanUndo,
  selectCanRedo,
  selectExperienceCount,
  selectEducationCount,
  selectSkillCount,
  selectProjectCount,
  selectCompletedSectionCount,
} from '../store/cvSelectors';
...
export function CVDataSummary() {
  ...
  const experienceCount = useCVStore(selectExperienceCount);
  const educationCount = useCVStore(selectEducationCount);
  const skillCount = useCVStore(selectSkillCount);
  const projectCount = useCVStore(selectProjectCount);
  const completedSectionCount = useCVStore(selectCompletedSectionCount);
    ...
    <div className="bg-surface rounded-xl border border-border p-5 space-y-4 shadow-2xs">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
        <div className="p-3 bg-app-bg rounded-lg border border-border/60 text-center">
          ...
          <span className="text-base font-bold text-foreground mt-0.5 block">{experienceCount}</span>
        </div>

        <div className="p-3 bg-app-bg rounded-lg border border-border/60 text-center">
          <span className="block text-foreground-secondary text-[11px]">{t('education')}</span>
          <span className="text-base font-bold text-foreground mt-0.5 block">{educationCount}</span>
        </div>

        <div className="p-3 bg-app-bg rounded-lg border border-border/60 text-center">
          <span className="block text-foreground-secondary text-[11px]">{t('skills')}</span>
          <span className="text-base font-bold text-foreground mt-0.5 block">{skillCount}</span>
        </div>

        <div className="p-3 bg-app-bg rounded-lg border border-border/60 text-center">
          <span className="block text-foreground-secondary text-[11px]">{t('projects')}</span>
          <span className="text-base font-bold text-foreground mt-0.5 block">{projectCount}</span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between text-[11px] text-foreground-secondary pt-1">
        ...
        <span>Completed: {completedSectionCount} sections</span>
      </div>
    </div>
  );
}
